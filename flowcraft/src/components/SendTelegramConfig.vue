<template>
  <div class="tg-config">
    ...
    <div class="form-group">
      ...
      />
      <div class="form-hint">可使用 <code>{{變數名}}</code> 引用上游節點的輸出</div>
    </div>
...
function deleteBot(i: number) { bots.value.splice(i, 1); if (selectedBotIdx.value >= bots.value.length) selectedBotIdx.value = 0 }
function saveBot() {
...
function deleteChat(i: number) { chats.value.splice(i, 1); if (selectedChatIdx.value >= chats.value.length) selectedChatIdx.value = 0 }
function saveChat() {
...
watch([bots, chats, selectedBotIdx, selectedChatIdx, localMessage, hiddenPortsList], emitUpdate, { deep: true })
</script>
